...
function show_activity_list(rsl_dict, ssl_dict,  nomal_dict){
	...
	window.load_gm_template("exchange_active_info/activity_list").done(function(tpl){
...
function show_info(activity_id){
	...
	var gift_list = activity.arl;
	var gold_list = activity.apl; 
	...
	for (var i = 0 ; i < gold_list_len; i ++){
		...
		for(var k = 0 ; k < goods_list_len; k ++){
			...
			display_list.push({gold:gold, goods_type:goods_item[0], id:goods_item[1]});
		}
	} 
	 
	
	window.load_gm_template("exchange_active_info/info").done(function(tpl){
		
		var doTpl = doT.template(tpl);
		var container = doTpl({"list":display_list});
		$("body").append(container);
		
		container = $("#exchange_active_list_container");
		
		container.dialog({title:"充值活动详细",close:function(){$("#exchange_active_list_container").remove();}});
